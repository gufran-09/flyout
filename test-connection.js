...
async function testConnection() {
  ...
  if (error) {
    ...
    console.log('Connected! Product Count:', data ? 0 : 'N/A', 'Total count from meta:', error ? 'N/A' : data);
    // Actually get one product
    const { data: p } = await supabase.from('products').select('id, title, thumbnail_image').limit(1);

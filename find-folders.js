...
async function findCategoryFolder() {
  const categoriesToCheck = ['Yacht', 'Water Adventures', 'Holiday Package', 'Cruises', 'Safari', 'Hotel', 'Car Rental'];
  const folderVariations = {
    'Yacht': ['Yacht', 'Yachts'],
    'Water Adventures': ['Water Adventures', 'Water Adventure', 'Water adventures', 'water adventure', 'Water-Adventures', 'water-adventure'],
    'Holiday Package': ['Holiday Package', 'Holiday-Package', 'Packages', 'Holiday Packages', 'Holiday-Packages', 'Holiday-Packages-In-Dubai'],
  ...
  for (const cat of categoriesToCheck) {
    ...
    for (const v of variations) {
      ...
      if (matchCount > 0) {
        console.log(`MATCH FOUND! Category [${cat}] uses folder [${v}] (${matchCount}/${qData.length} products matched)`);
        catMatch = v;
        break;
      }
    }

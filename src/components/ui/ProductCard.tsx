    ...
    >
      ...
      <div className="relative aspect-[4/5] overflow-hidden">
        ...
        <div className="absolute top-4 left-4 flex flex-col gap-2 items-start z-10">
          {tag && (
            <span
              className="px-3 py-1 bg-white/90 backdrop-blur-md shadow-sm rounded-full 
              text-[10px] font-medium uppercase tracking-widest text-flyout-gold ring-1 ring-white/50 flex items-center gap-1.5"
            >
              {tag === "Best Seller" && (
                <Flame className="w-3 h-3 text-orange-500/80" />
              )}
              {tag}
            </span>
          )}
        ...
        {category && (
          <div className="absolute top-4 right-4">
            <span
              className="px-3 py-1 bg-white/90 backdrop-blur-sm shadow-sm rounded-full 
              text-[10px] font-medium uppercase tracking-widest text-neutral-600 ring-1 ring-white/50"
            >
              {(category.toLowerCase() === "water-sports" ||
              category.toLowerCase() === "water sports"
                ? "Water Adventures"
                : category
              ).replace(/-/g, " ")}
        ...
        {booked && (
          <div className="absolute bottom-4 left-4">
            <span
              className="px-3 py-1 bg-white/80 backdrop-blur-md rounded-full 
      ...
      <div className="flex flex-col flex-grow p-6">
        ...
        {subtitle && (
          <p className="text-sm text-neutral-500 mb-5 line-clamp-2 font-light">
            {subtitle}
          </p>
        )}

        {location && !subtitle && (
          <p className="text-sm text-neutral-500 mb-5 line-clamp-1 font-light">
            {location}
          </p>
        )}

...
export default function SupplierSelector({ suppliers, onSelectionChange }: SupplierSelectorProps) {
  const activeSuppliers = useMemo(() => suppliers.filter((s) => s.is_active), [suppliers])
  const [selectedSupplierId, setSelectedSupplierId] = useState(activeSuppliers[0]?.id ?? '')

  const selectedSupplier = activeSuppliers.find((s) => s.id === selectedSupplierId)
  const activePricing = selectedSupplier?.product_pricing.filter((p) => p.is_active) ?? []
  const [selectedPricingId, setSelectedPricingId] = useState(activePricing[0]?.id ?? '')
  ...
  useEffect(() => {
    ...
    const firstPricing = selectedSupplier.product_pricing.find((p) => p.is_active)
    if (firstPricing && !selectedSupplier.product_pricing.some((p) => p.id === selectedPricingId && p.is_active)) {
    ...
    <div className="mt-6">
      ...
      <div className="grid grid-cols-1 gap-3 mt-3">
        {activeSuppliers.map((supplier) => {
          const supplierPrices = supplier.product_pricing.filter((p) => p.is_active).map((p) => p.price)
          const lowestPrice = supplierPrices.length > 0 ? Math.min(...supplierPrices) : supplier.price
          ...
          return (
            ...
            >
              <div className="flex justify-between items-start gap-4">
                <div>
                  <p className="font-semibold">{supplier.display_title ?? supplier.supplier?.name}</p>
                  {supplier.variant_description && (
                    <p className="text-sm text-gray-500 mt-1">{supplier.variant_description}</p>
                  )}
                  {supplier.location && <p className="text-xs text-gray-400 mt-1">{supplier.location}</p>}
                </div>
                <div className="text-right">
                  <p className="font-bold text-[#1A2B47]">From AED {lowestPrice.toLocaleString()}</p>
              ...
              <div className="flex gap-2 mt-3 flex-wrap">
                {supplier.product_pricing
                  ...
                  .map((pricing) => (
                    ...
                    >
                      {formatDuration(pricing.duration_minutes)} - AED {pricing.price.toLocaleString()}
                    </button>
                  ))}
              </div>
            </div>

...
export default function ZoneCard({ zone, side }: ZoneCardProps) {
  return (
    <div
      className={`relative max-w-md ${side === "left" ? "ml-auto" : "mr-auto"}`}
    >
      {/* CONNECTOR LINE */}
      <span
        className={`hidden lg:block absolute top-1/2 w-24 h-[2px] bg-sky-400/50 ${
          side === "left" ? "right-[-100px]" : "left-[-100px]"
        }`}
      >
        <span
          className={`absolute top-1/2 -translate-y-1/2 w-2 h-2 bg-sky-400 rounded-full shadow-[0_0_10px_rgba(56,189,248,0.8)] ${side === "left" ? "right-0" : "left-0"}`}
        />
      </span>
        ...
        {zone.ticketExamples && (
          <div className="mt-5 pt-4 border-t border-white/10 flex flex-wrap gap-2">
            {zone.ticketExamples.map((t, i) => (
              <span
                key={i}
                className="px-3 py-1 text-xs rounded-full bg-sky-500/10 border border-sky-500/20 text-sky-300 flex items-center gap-1 group-hover:bg-sky-500/20 transition-colors"
              >

...
import {
  User, Calendar, Heart, History, Settings, LogOut,
  MapPin, Clock, CheckCircle, XCircle, Loader2,
  Shield, Mail, ChevronRight, Star, Package,
  Bell, Edit3, Camera, Phone, Globe, Compass,
  TrendingUp, Award, CreditCard, AlertCircle
} from "lucide-react";
...
const PAYMENT_CONFIG = {
  pending:  { label: "Awaiting Payment", bg: "bg-amber-50",   text: "text-amber-700" },
...
function BookingCard({ booking }: { booking: Booking }) {
  ...
  const thumbnail = booking.product_supplier?.thumbnail_url
  ...
  return (
    ...
    >
      ...
      <div className="relative w-full sm:w-28 h-24 rounded-xl overflow-hidden flex-shrink-0">
        <img src={thumbnail} alt={productTitle} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
        <div className={`absolute top-2 left-2 flex items-center gap-1.5 px-2 py-0.5 rounded-full text-xs font-semibold ${status.bg} ${status.text}`}>
      ...
      <div className="flex-1 min-w-0">
        <h3 className="font-semibold text-[#1A2B47] truncate">{productTitle}</h3>
        <div className="flex flex-wrap gap-x-4 gap-y-1 mt-2 text-xs text-gray-500">
          <span className="flex items-center gap-1">
            <Calendar className="w-3 h-3" />{formatDate(booking.booking_date)}
          </span>
          <span className="flex items-center gap-1">
            <User className="w-3 h-3" />{booking.guests} guest{booking.guests > 1 ? "s" : ""}
          </span>
          {booking.product_pricing?.label && (
            <span className="flex items-center gap-1">
              <Clock className="w-3 h-3" />{booking.product_pricing.label}
            </span>
          )}
        </div>
      </div>
      ...
      <div className="flex sm:flex-col items-center sm:items-end justify-between sm:justify-start gap-2 flex-shrink-0">
        <p className="font-bold text-[#1A2B47]">AED {Number(booking.total_price).toLocaleString()}</p>
        <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${payment.bg} ${payment.text}`}>
          {payment.label}
        </span>
        {slug && (
          <Link href={href} className="text-xs text-[#D4A853] hover:underline flex items-center gap-0.5 mt-1">
  ...
  return (
    ...
    >
      <div className="relative h-44 overflow-hidden">
        <img
          src={product.thumbnail_url ?? "https://images.unsplash.com/photo-1512453979798-5ea266f8880c?w=400&q=80"}
          alt={product.title}
      ...
      <div className="p-4">
        <h3 className="font-semibold text-[#1A2B47] line-clamp-1 text-sm">{product.title}</h3>
        {product.location && (
...
function EmptyState({ icon: Icon, title, message, href, cta }: {
  icon: any; title: string; message: string; href?: string; cta?: string
}) {
  return (
...
export default function Dashboard() {
  ...
  const handleUpdateProfile = async (e: React.FormEvent) => {
    ...
    else { toast.success("Profile updated!"); fetchUserData(); }
  };

  const handleRemoveWishlist = async (id: string) => {
    const { error } = await supabase.from("wishlist").delete().eq("id", id);
    if (error) toast.error("Failed to remove");
    else { toast.success("Removed from wishlist"); fetchUserData(); }
  };
      ...
      <div className="min-h-screen bg-[#F8F9FB]">
        ...
        <div className="bg-[#1A2B47] relative overflow-hidden">
          ...
          <div className="absolute inset-0 opacity-10"
            style={{ backgroundImage: "radial-gradient(circle at 20% 50%, #D4A853 0%, transparent 50%), radial-gradient(circle at 80% 20%, #4A90D9 0%, transparent 40%)" }} />
          ...
          <div className="max-w-6xl mx-auto px-4 py-8 relative z-10">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  <p className="text-[#D4A853] text-xs font-medium tracking-widest uppercase mb-0.5">Welcome back</p>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-8">
              {[
                { label: "Active Bookings",    value: activeBookings.length,   icon: Calendar,   color: "text-blue-300" },
                { label: "Saved Experiences",  value: wishlist.length,          icon: Heart,      color: "text-rose-300" },
                { label: "Trips Completed",    value: completedBookings.length, icon: CheckCircle,color: "text-emerald-300" },
                { label: "Total Spent",        value: `AED ${totalSpent.toLocaleString()}`, icon: CreditCard, color: "text-[#D4A853]" },
              ].map((stat, i) => (
                <motion.div
                  key={stat.label}
          ...
          <AnimatePresence mode="wait">
            ...
            {activeTab === "bookings" && (
              <motion.div key="bookings" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
                <div className="flex items-center justify-between mb-6">
                  <h2 className="text-xl font-bold text-[#1A2B47]">Active Bookings</h2>
                  {activeBookings.length > 0 && (
                    <span className="text-sm text-gray-400">{activeBookings.length} booking{activeBookings.length > 1 ? "s" : ""}</span>
                  )}
                </div>
                {activeBookings.length === 0 ? (
            ...
            {activeTab === "wishlist" && (
              <motion.div key="wishlist" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
                <div className="flex items-center justify-between mb-6">
                  <h2 className="text-xl font-bold text-[#1A2B47]">Saved Experiences</h2>
                  {wishlist.length > 0 && (
                ...
                {wishlist.length === 0 ? (
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                    <AnimatePresence>
                      {wishlist.map(item => (
                        <WishlistCard key={item.id} item={item} onRemove={handleRemoveWishlist} />
                      ))}
                    </AnimatePresence>
                  </div>
                )}
            ...
            {activeTab === "history" && (
              <motion.div key="history" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
                <div className="flex items-center justify-between mb-6">
                  <h2 className="text-xl font-bold text-[#1A2B47]">Booking History</h2>
                  {completedBookings.length > 0 && (
                    <span className="text-sm text-gray-400">{completedBookings.length} completed</span>
                  )}
                </div>
                {completedBookings.length === 0 ? (
                  <div className="bg-white rounded-2xl border border-gray-100 shadow-sm">
            ...
            {activeTab === "profile" && (
              <motion.div key="profile" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
                <h2 className="text-xl font-bold text-[#1A2B47] mb-6">Profile Settings</h2>
                    ...
                    <div className="relative inline-block mb-4">
                      {profile?.avatar_url ? (
                        ...
                        <div className="w-24 h-24 rounded-2xl bg-gradient-to-br from-[#1A2B47] to-[#2C4A7C] flex items-center justify-center mx-auto">
                          <span className="text-3xl font-bold text-[#D4A853]">{initials}</span>
                        </div>
                      )}
                    </div>
                    <h3 className="font-bold text-[#1A2B47] text-lg">{displayName}</h3>
                  ...
                  <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 lg:col-span-2">
                    ...
                    <form onSubmit={handleUpdateProfile} className="space-y-5">
                      ...
                      <div>
                        ...
                        <Input
                          id="full_name" type="text" value={formData.full_name}
